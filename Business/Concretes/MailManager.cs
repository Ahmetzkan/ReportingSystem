...
namespace Business.Concretes
{
    public class MailManager : IMailService
    {
        ...
        public async Task<bool> SendMailAsync(SendMailRequest sendMailRequest)
        {
            ...
            mail.From = new(_configuration.GetSection("Smtp:Username").Value?.ToString(), "Reporting System", System.Text.Encoding.UTF8);
        ...
        public async Task SendPasswordResetMailAsync(SendPasswordResetMailRequest sendPasswordResetMailRequest)
        {
            ...
            mail.AppendLine($"Merhaba <br> Yeni şifre talebinde bulundaysanız aşağıdaki linkten şifrenizi yenileyebilirsiniz." +
                $" <br><strong><a target=\"_blank\" href=\"http://localhost:44355/reset-password/" +
                $"{Uri.EscapeDataString(sendPasswordResetMailRequest.UserId.ToString())}/" +
                $"{Uri.EscapeDataString(sendPasswordResetMailRequest.ResetToken)}\">" +
                $" Yeni şifre talebi için tıklayınız.. </a></strong><br><br><span style=\"font-size:12px;\">" +
                $" NOT : Eğer ki bu talep size ait değilse bu maili görmezden geliniz.");
